...
public class LinkInfoServiceImpl implements LinkInfoService {
    ...
    @Autowired
    public LinkInfoServiceImpl(LinkInfoRepository linkInfoRepository, LinkShortenerProperty linkShortenerProperty, LinkInfoMapper linkInfoMapper) {
    ...
    public List<CreateShortLinkResponse> getAllShortLinks() {
        return linkInfoRepository.findAllShortLinks()
                .stream().map(linkInfoMapper::toResponse).toList();
    }


    public CreateShortLinkResponse deleteById(UUID id) {
        return linkInfoMapper.toResponse(linkInfoRepository.deleteById(id)
                .orElseThrow(() -> new NotFoundException(
